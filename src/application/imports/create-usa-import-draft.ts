...
import {
  ...
  type ParsedUsaDraftItem,
} from "./usa-monitoring-schema";
import type {
  ImportCandidateInspection,
  ImportDraftItemInput,
  MonitoringImportRepository,
} from "./types";
import type { CreateImportDraftResult } from "./create-import-draft";
...
function reviewItem(
  ordinal: number,
  parsed: ParsedUsaDraftItem,
  inspection: ImportCandidateInspection | null,
): ImportDraftItemInput {
  const company = parsed.company;
  if (!company) {
    const identity = parsed.rawCompany.identity;
    return {
  ...
  return {
    ...
    errors,
    normalizedPayload:
      company as unknown as ImportDraftItemInput["normalizedPayload"],
  };
}

export async function createUsaMonitoringImportDraft({
  repository,
  userId,
  payload,
  fileName,
  ...
  const parsed = parseUsaMonitoringImportDraft(payload);
  if (!parsed.success) {
    return {
      status: "invalid_file",
  ...
  const externalId =
    ...
    `usa-import-${parsed.data.analysisDate}-${payloadHash}`;
  const generatedAt =
    parsed.data.generatedAt ?? `${parsed.data.analysisDate}T00:00:00.000Z`;
