alter table public.monitoring_import_batches
  drop constraint monitoring_import_batches_export_type_check;

alter table public.monitoring_import_batches
  add constraint monitoring_import_batches_export_type_check check (
    export_type in ('gpw_opportunity_monitoring', 'usa_opportunity_monitoring')
  );

create or replace function public.create_usa_monitoring_import_draft(
  p_external_id text,
...
begin
  ...
  insert into public.monitoring_import_batches (
    ...
    v_user_id, p_external_id, '1.0', 'usa_opportunity_monitoring',
...
revoke all on function public.create_usa_monitoring_import_draft(
  text, timestamptz, date, text, jsonb, integer, jsonb
) from public, anon;
grant execute on function public.create_usa_monitoring_import_draft(
  text, timestamptz, date, text, jsonb, integer, jsonb
) to authenticated;

create or replace function public.commit_usa_monitoring_import_item(
  p_item_id uuid,
...
begin
  ...
  where id = v_item.batch_id
    ...
    and export_type = 'usa_opportunity_monitoring'
  for update;
  if not found then
    return query select 'invalid_item'::text, null::uuid, null::uuid;
    return;
  ...
  select id into v_market_id from public.markets where code = 'USA';
  ...
  insert into public.monitoring_results (
    ...
    coalesce(v_company #> '{thesis,pros}', '[]'::jsonb),
    coalesce(v_company #> '{thesis,risks}', '[]'::jsonb),
  ...
  insert into public.investment_theses (
    ...
    coalesce(v_company #> '{thesis,catalysts}', '[]'::jsonb),
    coalesce(v_company #> '{thesis,risks}', '[]'::jsonb),
    coalesce(v_company #> '{thesis,killCriteria}', '[]'::jsonb),
    coalesce(v_company #> '{thesis,growthDrivers}', '[]'::jsonb),
    coalesce(v_company #> '{thesis,epsFcfGrowthDrivers}', '[]'::jsonb),
    coalesce(v_company #> '{thesis,pros}', '[]'::jsonb)
  );
  ...
  loop
    ...
    from jsonb_array_elements(coalesce(v_company #> '{positionPlan,tranches}', '[]'::jsonb))
...
revoke all on function public.commit_usa_monitoring_import_item(uuid, jsonb) from public, anon;
grant execute on function public.commit_usa_monitoring_import_item(uuid, jsonb) to authenticated;
